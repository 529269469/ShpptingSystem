...
import com.fhxh.shpptingsystem.utils.ByteUtil;
 ...
 * 串口实处理类
 */
public class SerialHandle2 implements Runnable {
    ...
    private void readData() {
        ...
        readTask = SerialManage
                ...
                .scheduleAtFixedRate(this, 0, 10, TimeUnit.MILLISECONDS);//执行一个循环任务
    }

    @Override//每隔 100 毫秒会触发一次run
    public void run() {
    ...
    public void send(final String msg) {
        Log.e(TAG, "靶send: " + msg);
        byte[] bytes;
        if (msg.contains("90AA55")||msg.contains("C00009000108E600")) {
            bytes = hexStr2bytes(msg);
        } else {
            bytes = ByteUtil.hexStr2bytes(ByteUtil.str2HexString(msg));
        }
        try {
